...
import (
	...
	"wondee/finance-app-backend/internal/auth"
	"wondee/finance-app-backend/internal/storage"
	usermodel "wondee/finance-app-backend/internal/user"
...
func (h *AuthHandler) Callback(c *gin.Context) {
	...
	if err != nil {
		...
		if workspaceID == 0 {
			...
			if err := h.Repo.CreateWorkspace(ws); err != nil {
		...
		defaultProfile := &wealth.WealthProfile{
			UserID:                user.ID,
			WorkspaceID:           workspaceID,
	...
	tokenString, err := auth.GenerateJWT(user.ID, user.WorkspaceID, secret, 24*time.Hour)
	if err != nil {
...
func (h *AuthHandler) Me(c *gin.Context) {
	...
	secret := os.Getenv("JWT_SECRET")
	claims, err := auth.ValidateJWT(cookie, secret)
	if err != nil {
