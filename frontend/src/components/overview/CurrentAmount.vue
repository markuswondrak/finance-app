<template>
  <v-banner sticky elevation="4" :icon="icon" :color="currentAmount >= 0 ? 'surface-bright' : 'surface'">
    <template v-slot:text>
      <span class="text-body-2 text-medium-emphasis mr-2">Aktueller Betrag:</span>
      <strong :class="[amountColorClass, 'text-h4', 'font-weight-bold']">{{ toCurrency(currentAmount) }}</strong>
    </template>
    <template v-slot:actions>
      <v-btn variant="text" color="primary" @click="show = true">Ändern</v-btn>
    </template>
    <v-dialog v-model="show" max-width="600">
      <v-card class="card-accent-primary" rounded="lg" elevation="4">
...
export default {
  ...
  computed: {
    ...
    amountColorClass() {
      if (this.currentAmount > 0) return 'text-success';
      if (this.currentAmount < 0) return 'text-error';
      return 'text-neutral';
    }
  },
  data() {
